...
const mapPaths = ['./map/latar.png', './map/latar2.png', './map/latar3.png', './map/latar4.png', './map/latar5.png'];
...
(function(){
  ...
  // Hide loading screen immediately when coming from virtual tour
  if (loadingScreen) { 
    loadingScreen.style.display = 'none';
  }
  
  // No setTimeout delay for virtual tour mode
  if (fightingMode === 'fromVirtualTour') {
    ...
    // Initial load or from menu - show character selection with delay
    setTimeout(() => {
...
function showMapLoadingScreen(mapIndex, callback) {
  // Map index to loading screen image
  const loadingImages = [
    './loading/loadcangkuang.png',  // 1: latar.jpg
    './loading/loadborobudur.png',  // 2: latar2.png
    './loading/loadgerbang.png',    // 3: latar3.png
    './loading/loadprambanan.png',  // 4: latar4.png
    './loading/loadsambisari.png'   // 5: latar5.png
  ];
  
  const loadingImg = loadingImages[mapIndex - 1] || loadingImages[0];
  
  // Create loading screen overlay
  const loadingDiv = document.createElement('div');
  loadingDiv.id = 'map-loading-screen';
  ...
  `;
  loadingDiv.appendChild(promptText);
...
function startMatch() {
  ...
  const currentMapIndex = mapPaths.indexOf(selectedMap) + 1;
  
  // Show loading screen based on map, then start round
  showMapLoadingScreen(currentMapIndex, startRound);
}
...
function resolveWinner(forcedWinner) {
  ...
  if (matchEnded) {
    ...
    // Auto return to virtual tour after match ends
    setTimeout(() => {
      localStorage.removeItem('fightingGameMode');
      window.location.href = './index.html';
    }, 3000); // 3 seconds delay to show winner message
  } else {
