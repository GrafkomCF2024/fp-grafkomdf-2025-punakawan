...
function rebuildLighting() {
  ...
  try {
    ...
    const sunGeom = new THREE.SphereGeometry(12, 16, 12);
  ...
  try {
    ...
    sunSprite = makeGlowSprite(0xffee88, 256, 1.0, 0.0);
    sunSprite.scale.set(32, 32, 1);
    sunSprite.renderOrder = 1001;
    sunSprite.position.copy(sunLight.position);
    scene.add(sunSprite);

    sunHalo = makeGlowSprite(0xffaa66, 256, 0.6, 0.0);
  ...
  try {
    ...
    const moonGeom = new THREE.SphereGeometry(8, 12, 12);
  ...
  try {
    ...
    moonSprite = makeGlowSprite(0xddeeff, 192, 0.9, 0.0);
    ...
    moonHalo = makeGlowSprite(0x99bbff, 256, 0.35, 0.0);
...
function updateSunPosition(timeValue) {
  ...
  try {
    ...
    // Update shadow camera after moving light/target
    if (sunLight.shadow && sunLight.shadow.camera) sunLight.shadow.camera.updateProjectionMatrix();
  } catch (e) { /* non-fatal */ }
